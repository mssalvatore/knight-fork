...
;(function () {
  ...
  function constructor (containerElOrString, config) {
    ...
    function mouseclickSquare (evt) {
      ...
      if (!validSquare($(square).attr(('data-square')))) return
    
      config.onMouseclickSquare(evt, square)
    }

    function contextmenuSquare (evt) {
      // do not fire this event if we are dragging a piece
      // NOTE: this should never happen, but it's a safeguard
      if (isDragging) return

      // exit if they did not provide a onMouseoverSquare function
      if (!isFunction(config.onContextmenuSquare)) return
      ...
      config.onContextmenuSquare(evt, square)
    }
    ...
    function addEvents () {
      ...
      $board
        .on('click', '.' + CSS.square, mouseclickSquare)
        .on('contextmenu', '.' + CSS.square, contextmenuSquare)
  ...
  window['Chessboard']['validSquare'] = validSquare
